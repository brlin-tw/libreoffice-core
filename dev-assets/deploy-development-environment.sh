...
setup_build_user(){
    ...
    local regex_cc_max_size='^max_size[[:space:]]*=[[:space:]]*32G'
    local -a grep_opts=(
        -E
        -e "${regex_cc_max_size}"
        -q
    )
    if ! grep "${grep_opts[@]}" ~builduser/.config/ccache/ccache.conf; then
        printf \
            "Info: Setting the build user's Ccache max cache size to 32GiB...\\n"
        if ! sudo -u builduser ccache --max-size 32G; then
            printf \
                "Error: Unable to set the build user's Ccache max cache size to 32GiB...\\n" \
                1>&2
            return 2
        fi
    fi
